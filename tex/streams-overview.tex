...
Views of such detailed substructure in galactic halos have so far been seen only in numerical simulations, but every point here represents a high-probability member star of a Milky Way stream identified in \citet{ibata:2024, li:2019, vasiliev:2021}.
\label{fig:fos_3d}
}
\end{figure*}
...
In such a potential, it is possible to simplify the equations of motions using the Hamiltonian canonical variables \citep{goldstein:2002, eyre:2011}:
\begin{align}
...
- orbital phase space from the stream population, comparison to satellites, diffuse halo stars
- first w pms: orbital poles
- why E Lz and not action -- instantaneous properties, can be determined for every potential
- full orbits: phase-space clustering \citep{bonaca:2021}
...
- cetus-palca (yuan, chang, thomas)
- may even explain the long-standing puzzle of the sgr bifurcation (penarrubia, recent disk claim) as group infall (davies)
- accretion origin important for cocoon implications (carlbergs, malhans)
- velocity dispersion, pm and rv (malhans)
...
% - side benefit: can use orbits for membership selection
