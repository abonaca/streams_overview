...
Views of such detailed substructure in galactic halos have so far been seen only in numerical simulations, but every point here represents a high-probability member star of a Milky Way stream identified by \citet{ibata:2024, li:2019, vasiliev:2021}.
...
\begin{figure}[t!]
    ...
    with each of the streams in \citet{ibata:2024} (this procedure is described in
...
- shift: we have gone from interpreting vr gradients to measuring orbits -- provides additional context
...
Widely available kinematics allow us to accurately determine the streams' orbits, which inform us about the streams' dynamical histories and processes likely to have produced features discussed in \S\ref{sec:structure}.
...
Simultaneous modeling of multiple globular cluster streams in the inner $\lesssim20$\,\unit{\kpc} found an ellipsoidal halo model a reasonably good fit, and thanks to \gaia\ kinematics put strong constraints on the prolate halo shape \citep[$q_z=1.06\pm0.06$;][]{palau:2023}, and the halo virial mass \citep[$M_{200}=1.09^{+0.19}_{-0.14}\times10^{12}\,\unit{\msun}$;][see also \citealt{reino:2021}]{ibata:2024}.
However, reproducing the positions and kinematics of the more distant Sagittarius stream requires a more complex, time-dependent model with an oblate inner halo, triaxial outer halo, as well as a massive LMC on its first infall \citep{vasiliev:2021}.
This change in halo shape is likely due to the LMC deforming the Milky Way's outer halo \citep[e.g.,][]{garavito-camargo:2019,shao:2021,vasiliev:2023}.
...
\gaia~\dr{3} improved proper motions and orbital constraints of the halo streams to allow placing them on the $E-Lz$ plane (see Figure~\ref{fig:phase_space}, right).
Streams are highly clustered in the $E-L_z$ space (also seen as prominent clusters in orbital eccentricities, Figure~\ref{fig:phase_space} left), and aligned with debris from identified Milky Way progenitors \citep{bonaca:2021,malhan:2022}.
...
Knowledge of these hierarchical relations between different streams can even directly test dark matter on small scales.
Numerical simulations show that tidal debris from the dwarf galaxy itself has a lower velocity dispersion if the galaxy is cuspy \citep{errani:2015}, due to its more compact size.
...
Observational data for individual streams currently lack the precision to put strong constraints \citep{malhan:2022b}, but systems where both the progenitor and the satellite streams are present provide a new opportunity to build self-consistent models that accurately constrain the inner slope of the accreted dark matter halos.

% - reconstruction helpful in estimating what internal, vs external MW perturbation
% - but ex-situ gc streams also a more direct test of dm on small scales, specifically the inner density slope of their progenitor dwarf galaxy
% - cuspy -- higher velocity dispersion, lower for a less disruptive cored profile \citep{malhan:}
% - data not there yet for individual streams, but may be possible to built a self-consistent model for a more complex system where both the dwarf galaxy and the globular cluster stream are present.
% Building self-consistent models of group infall events will not only produce the full merger tree of the Milky Way, but also has the potential

% N-body simulations of globular clusters evolved in dark matter halos that were subsequently accreted onto a Milky Way-like host halo produce stellar streams surrounded by a range of low density features \citep[e.g., wide envelopes, or cocoons, and sub-streams;][]{carlberg:2018,carlberg:2020,malhan:2019a,qian:2022}.
% Similar off-stream features can be produced by an impact of a dark-matter subhalo in the Milky Way \citep[e.g.,][]{bonaca:2019}, but for ex-situ globular cluster progenitors they could also originate from stars lost before the accretion onto the Milky Way.
% , and are likely segments of the extended tidal tails of NGC~5824 or another fully disrupted cluster
% , and are likely segments of its extended tidal tails 
% - possible evidence: cetus-palca (yuan, chang, thomas)
% - may even explain the long-standing puzzle of the sgr bifurcation (penarrubia, recent disk claim) as group infall (davies)
% - also affects our interpretation of the measured velocity dispersion, pm and rv and interpretation on cusp vs core (malhans)


\subsection{Progenitors and early star formation}
In a few cases, the orbital phase space discovered the direct progenitor of a stellar stream previously thought completely dissolved.
Figure~\ref{fig:phase_space} also shows Milky Way globular clusters (light blue circles) and dwarf galaxies (dark blue circles), and some of them coincide with stellar streams in the $E-L_z$ plane--their progenitors.
- marked in the table, big increase w \gaia, otherwise have been hard to find \citep{balbinot}
In some cases, like between Gj\" ol and NGC~3201, the connection goes through the disk, so likely observational bias that not connected.
In other cases it is unclear if it's observational or a true gap that spans up to tens of degrees, which might indicate massive perturbation affected, or potentially episodic mass loss. 
Furthermore, many of these systems only have one tidal arm discovered, also an open question.
Despite these peculiarities, this subset of stellar streams with known globular cluster progenitors form a valuable sample for gravitational potential inference.
Controlling for the progenitor improves modeling \citep{kupper:2015}, otherwise a free parameter + can also fit the overdensities
Forecast epicycles can be taken into account when interpreting density variations as well \citep{ibata,}

% - not just connecting different orbital segments, but also to other halo denizens
% - figure: comparison to globular clusters (light blue circles) and dwarf galaxies (dark blue circles) in the Milky Way
% - progenitors important: can model much better 
% - before: only 2 gc streams, hard to find \citep{balbinot}

- corollary of widely available spectra of are chemical abundances
- powerful in discerning origins, and even finding what is not an accreted stream but in-situ (e.g., Monoceros and Nyx)
- among accreted: GC vs DG streams from metallicity spread (Ji S5)
- sgr detailed abundances, older wraps (Johnson H3)
- other dg streams? (e.g., lms, malhan, yuan)

- detail abundances of GC streams used to firm up the link w the progenitor cluster, e.g., gjoll--ngc3201 chemistry \citep{hansen:2020}
- used to trace the contribution of GCs to the bulge / halo budget
...
- potential to test how universal by extend beyond the MW  (pearson; aganze)
