...
However, it is conceptually useful to distinguish the dominant processes that can impact
...
We reiterate that these processes are not independent, but

\subsubsection{Disruption and Orbit of the Progenitor}

% Stream on near-circular orbit vs. very eccentric
- Shocks vs. slow two-body relaxation
- Black holes (Gieles)
- Stream vs. shell
- Hendel paper on debris morphology

\subsubsection{Epicyclic Over-densities}

- Kuepper

\subsubsection{Orbital Resonances and Structure}

- Sarah
- APW
- Tomer

\subsubsection{Interactions with the Bar and Spiral Arms}

- APW (Ophiuchus)
- Kohei Hattori
- Spiral arms??

\subsubsection{Interactions with Darm Matter Subhalos}

- KVJ, Ibata, Hoon, Ana Via Lactea work

\subsubsection{Interactions with Massive Dwarf Galaxies}

- Sag (dillamore)
- LMC (many recent papers)


\subsection{Observed Structures: A Few Case Studies}

All of the ($\lesssim$10 \todo{made up this number}) streams that have been studied in
detail show evidence for a complex dynamical history in the form of density features
along and around the main stream tracks.
We mainly focus here on the low-mass (likely globular-cluster origin) streams, but even
the Sagittarius stream shows a clear bifurcation in the density of stars along the
extent of the stream (see, e.g., Figure~XX in \citealt{todo}).
The streams below are some of the most well-studied streams around the Milky Way, but as
we discuss in the next section, theoretical models still struggle to quantitatively
explain the observed complexity of these streams.
