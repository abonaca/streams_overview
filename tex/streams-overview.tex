...
The \gaia\ mission has mapped the positions and motions of almost two billion stars in the Milky Way \citep{gaiamission:2016, gaiadr1, gaiadr2, gaiaedr3, gaiadr3}, so it comes as no surprise that it has fundamentally changed how we view groups of stars that move on similar orbits---stellar streams.
% Formed through tidal dissolution of globular cluster and dwarf galaxy satellites.
...
% With its measurement of proper motions, \gaia\ lifted the main limitation in studies of stellar streams: uncovering member stars of these low-mass and diffuse systems in the ocean of field Milky Way stars.
% As a result, maps of stellar streams in the \gaia\ era build a more complete picture of our Galaxy, its constituents and its history.

We center this review on three major breakthroughs that \gaia\ brought to stellar streams.
The first is the sheer number of stream discoveries, which increased from $\approx10$ to $\approx100$ thanks to \gaia's capability to detect comoving, diffuse groups of stars (\S\ref{sec:discovery}).
The second are the stream kinematics, which constrained their precise orbits and origins (\S\ref{sec:orbits}).
...
- we expect these to have set in motion future progress as well, and discuss key opportunities for breakthroughs for each of the major topics, and reflect on changes awaiting us in the future due to these discoveries in the concluding section
As the last review dedicated to stellar streams was almost a decade ago and pre-\gaia\ \citep{newberg:2016}, while a more recent, \gaia-based review focused on the Milky Way's halo more broadly \citep{helmi:2020}, in the remainder of this section we provide the theoretical and historical context for \gaia's stellar stream revolution.
...
When sufficiently strong, these tides pull out stars through two stationary points to form the leading and trailing tidal tails \citep{bt:2008}.
We illustrate the process of tidal dissolution in Figure~\ref{}, with a dwarf galaxy satellite at the top, and a globular cluster on a matching orbit at the bottom.
The left-most panels show an early stage of dissolution with short tails of gravitationally unbound stars emanating from the progenitor.
...
\end{equation}
Tidal radius is shown with a red dashed line in Figure~\ref{}, and as expected is 10 times smaller for the $10^4\,\unit{\msun}$ globular cluster than the $10^7\,\unit{\msun}$ dwarf galaxy.
...
Notably, both streams delineate the progenitor's orbit at pericenter, but are misaligned at apocenter \citep[left and middle panels in Figure~\ref{};][]{eyre,sanders}.
However, dwarfs galaxies are more diffuse than globular clusters, so their mass-loss rates are significantly higher and produce longer tails in the same amount of time.
Furthermore, dwarf galaxies predominantly lose mass during pericentric passages \citep{}, which results in tidal tails of a fairly uniform density (Figure~\ref{}, right).
In contrast, globular clusters \citep{}.

% It also decreases with the Galactocentric radius, $R_0$, which makes tidal stripping more efficient at pericenter.



% through the analogs of the L3 and L2 Lagrange points from the restricted three-body problem.
% - two tails: lagrange points
% - jacobi radius
% - orbit misalignment at peri vs apo
% - stream length
% - density structure (epicycles)
...
- now we've seen even more extragalactic streams -- interacting galaxies, streams, shells
-> clearly important process for understanding galaxy formation
