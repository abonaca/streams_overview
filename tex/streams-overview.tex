...
Third, stream kinematics are now widely available and have constrained the streams'
orbits and origins, both in-situ and accreted.
...
The Milky Way is currently in significant disequilibrium due to an undergoing a merger with the LMC \citep[e.g.,][]{besla:2007,kallivayalil:2013,vasiliev:2024}, which is expected to result in large variations of subhalo impacts on streams with different orbits \citep{barry:2023,arora:2023}.
...
This would perfectly complement constraints on the individual subhalo properties expected from modeling of stream impact sites, in large part because statistical properties of a population of subhalos are different from population properties of other sources of perturbation streams experience.
% Upcoming observational data sets will allow measurements of density profiles of most streams \citep[e.g., with Rubin LSST][]{}.
So far only used power-spectrum of density variations along stellar streams, but now see streams not 1D and expect to complement with large-scale kinematic maps, s of higher-order may be 
Fast methods for generating stream models are even more important for this line of work as it requires simulating the whole population of streams for a single likelihood call.

% - obvious challenge as in previous section is having fast stream models, even more important here as want to simulate multiple streams simultaneously

% may not provide as and potentially relieve some of the challenges in retrieving properties of dark-matter substructure from individual impacts.
...
We also thank Petra Awad, Eduardo Balbinot, Sergey Koposov, Jake Nibauer, and Kiyan Tavangar for
sharing data used to recreate figures from their work.
We additionally thank Charlie Conroy, Cecilia Mateu, Francois Schweizer, and \todo{who else?}
