...
\citet{caldwell:2020} showed that the Ophiuchus stream extends further on the sky than
previously thought.
But new constraints on the orbit of Ophiuchus from this work supports the idea that the
Galactic bar has significantly impacted the structure of the stream.
...
around the Milky Way \citep{odenkirchen:2001,rockosi:2002}.
Pal 5 serves as an archetype for globular cluster streams, as it is long ($\sim
7~\kpc$), thin (width standard deviation $\sigma \sim 4~\unit{\parsec}$), relatively
high surface brightness, still connects to its progenitor system, and has been studied
in detail since its discovery \citep[e.g.,][]{price-whelan:2019}.

- density variations
- asymmetry
- fan
- possible extension (Starkman)
...
In the previous section we discussed how different sources of stream perturbations often result in similar morphological features.
Because of this, none of the numerous features detected in the Milky Way streams has been unambiguously associated with a perturbation mechanism.
Here we discuss precision kinematics as a way forward, and demonstrate how they could distinguish between two scenarios for an origin of a stream gap: a dissolved progenitor and a dark-matter subhalo impact.
...
In the unperturbed stream, radial velocities are similar to the orbital trend, with only slight and monotonic offsets (middle right).
Subhalo impacts introduce non-monotonic features, which are more prominent for the perpendicular (middle left) than for the grazing impact (middle center).
...
Due to high scatter, the radial velocity signature of a perpendicular impact would be marginally resolved at the present (bottom left), while that of a grazing impact would be washed away and indistinguishable from the gap associated with the dissolved progenitor (bottom center).
...
Of course, Figure~\ref{fig:rv_signal} illustrates only a couple of impacts, and more extensive theoretical work is needed to determine subhalo detectability in the kinematic space as a function of the subhalo mass, size, impact geometry and time, as well as how distinguishable other perturbation mechanisms are from subhalo impacts.
...
% Additionally, concerted survey effort to measure these features on large scales.
% This is just an illustration, and more extensive theoretical work needed to determine when degeneracies are lifted, dependence on orbit, vr vs pm, off-stream modeling
% We expect similar signatures present for distinguishing other mechanisms, but more theoretical work needed for those too.
% Observationally, need to launch the campaigns to measure this / build instrumentation for it.
% Lots of work ahead, but worth it because this is a key opportunity to advance our understanding of stellar streams.



% Despite the gaps being similar in sky positions (top row), their radial velocities are different.
% The middle and bottom rows show radial velocities relative to the orbital trend, convolved with a measurement uncertainty of $2\,\unit{\kms}$ and $0.1\,\unit{\kms}$, respectively.


% Color-coded by how much of an outlier from a GP
% Present-day wide-field spectrographs typically approach $2\,\unit{\kms}$ (bottom row) allow to detect differences in 
% These differences are easily observed in the case of small observational uncertainties ($0.1\,\unit{\kms}$ in the middle row), how

% - previous section outlined various sources of perturbation, many of which result in similar morphological features
% - for example: a gap could be due 
% - similarly, a spur could be a part of a loop of stars kicked out of the stream, or a remnant of an earlier stripping episode
% - we here demonstrate how these different scenarios can be distinguished using precision kinematics

% - figure shows a GD-1 like stream with two prominent gaps, one originating from an impact, and one a site of a dissolved progenitor
% - while 
