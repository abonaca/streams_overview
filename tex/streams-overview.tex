...
Table~\ref{tbl:members} shows 20 rows for an abridged set of columns, while the full table is accessible on the project Github and Zenodo repositories.
}
...
\begin{figure*}[t!]
\begin{center}
\includegraphics[width=1\textwidth]{figures/all_members_sky.pdf}
\end{center}
\caption{%
\changes{
...
\changes{For each stream, we use} a heliocentric, spherical coordinate system that is approximately aligned with the stream track\changes{, reported in the \package{galstreams} package \citep{mateu:2023} and in the online version of Table~\ref{tbl:summary}}.
...
Columns include: $l$ on-sky length, $w$ on-sky width, $d_h$ heliocentric distance, $R_{gal}$ Galactocentric distance, $M_\star$ lower limit on the stellar mass of the stream, $R_{peri}$ pericenter, $R_{apo}$ apocenter, $e$ orbital eccentricity, $T_{orb}$ orbital period, $E_{tot}$ total orbital energy, $L_z$ and $L_\perp$ components of the orbital angular momentum perpendicular to and aligned with the Galactic disk, respectively.
