...
- but also more direct tracer of dm: distribution on large, galactic, and small, subgalactic, scales \citep{johnston, ibata}
...
- theory predictions: dm subhalo impacts produce a range of features \citep{carlberg, yoon}, but most ubiquitous an underdensity, or a gap.
...
These findings motivated deeper imaging of these streams, which confirmed several prominent, degrees-wide gaps, but also revealed a density profile that is overall more uniform than that inferred from the shallower imaging \citep{ibata:2016, erkal:2017, deboer:2018, bonaca:2020}.
...
% This signifies that at least some variations in the observed density of stream stars are due to contamination from the field stars, rather than intrinsic stream underdensities \citep{thomas:2016}.
% Deep photometry increases the stream contrast over the Milky Way field stars and allows for a robust test of dark-matter substructure, but is prohibitively expensive for the entire stream population even with today's facilities.
