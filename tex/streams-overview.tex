...
However, additional spectroscopy is needed to measure radial velocities and determine which dynamical processes were at play in shaping individual streams, from uncovering the origin of stream gaps (\S\ref{sec:}) to revealing the wide envelopes of earlier dissolution episodes (\S\ref{sec:}).
A number of spectroscopic surveys have been conducted in the past couple of decades, but they obtained a fairly small number of stream members because they are either too bright (e.g., Gaia \citealt{gaia}, APOGEE \citealt{majewski:}), and/or employ a sparse targeting strategy that misses the narrow streams' profile (e.g., SEGUE \citealt{segue}, H3 \citealt{conroy:2019}).
On the other hand, stream members can be efficiently targeted with a dedicated program using large-field, heavily multiplexed spectrographs \citep[e.g.,][]{li:2021}.
Excitingly, several such upcoming facilities have a dedicated stream component, e.g., WEAVE \citep{}, 4MOST \citep{}, PFS \citep{}, and others may have the sufficient survey speed to target all \gaia\ stars \citep[e.g., DESI,][]{desi}.
These surveys will complete the membership and constrain large-scale processes (e.g., halo shape, LMC perturbations), but lack resolution 
- high-res components are planned for, but inefficient for large
...
- streams being recognized as one in both physics and astro planning processes
...
- CATS proposal
- coordination (dual hemispheres), population studies
